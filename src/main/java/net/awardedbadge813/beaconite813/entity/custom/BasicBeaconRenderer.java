...
public class BasicBeaconRenderer implements BlockEntityRenderer<BeaconBeamHolder> {
    ...
    public void render(BeaconBeamHolder blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        if(blockEntity.getBeamSections()!=null){
            List<BeaconBeamHolder.BeaconBeamSection> list = blockEntity.getBeamSections();
            int j = 0;
                for (int k = 0; k < list.size(); k++) {
                    BeaconBeamHolder.BeaconBeamSection BasicBeaconBeamHolder$beaconbeamsection = list.get(k);
                    renderBeaconBeam(
                            blockEntity,
                            poseStack,
                            bufferSource,
                            ...
                            i,
                            j,
                            k == list.size() - 1 ? 1024 : BasicBeaconBeamHolder$beaconbeamsection.getHeight(),
                            BasicBeaconBeamHolder$beaconbeamsection.getColor()
                    );
                    j += BasicBeaconBeamHolder$beaconbeamsection.getHeight();
                }
        }
    ...
    private static void renderBeaconBeam(
            BeaconBeamHolder blockEntity, PoseStack poseStack, MultiBufferSource bufferSource, float partialTick, long gameTime, int yOffset, int height, int color
    ) {
        if(true) {
    ...
    public net.minecraft.world.phys.AABB getRenderBoundingBox(BeaconBeamHolder blockEntity) {
